...
def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode):
    ...
    all_tag_ids = torch.tensor([f.label_ids for f in features], dtype=torch.long)
    all_head_ids = torch.tensor([f.head_ids for f in features], dtype=torch.long)
    dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_tag_ids, all_head_ids)
    return dataset
...
def main():
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        model.to(args.device)
        result = evaluate(args, model, tokenizer, labels, pad_token_label_id, mode="test")
        # Save results
