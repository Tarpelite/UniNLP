...
from utils_parsing import convert_examples_to_features as convert_examples_to_features_parsing
from utils_parsing import read_examples_from_file as read_examples_from_file_parsing
...
def load_and_cache_dev_examples(args, tokenizer, pos_labels, ner_labels, chunking_labels, srl_labels, onto_pos_labels, onto_ner_labels, pad_token_label_id, is_ft=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        logger.info("Creating onto ner examples from dataset file at %s", args.onto_ner_data_dir)
        if is_ft:
    ...
    all_input_ids = torch.tensor([f.input_ids for f in parsing_ud_features], dtype=torch.long)
    all_input_mask = torch.tensor([f.input_mask for f in parsing_ud_features], dtype=torch.long)
    all_segment_ids = torch.tensor([f.segment_ids for f in parsing_ud_features], dtype=torch.long)
    all_label_ids = torch.tensor([f.label_ids for f in parsing_ud_features], dtype=torch.long)

    parsing_ud_dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids)

    all_input_ids = torch.tensor([f.input_ids for f in parsing_ptb_features], dtype=torch.long)
    all_input_mask = torch.tensor([f.input_mask for f in parsing_ptb_features], dtype=torch.long)
    all_segment_ids = torch.tensor([f.segment_ids for f in parsing_ptb_features], dtype=torch.long)
    all_label_ids = torch.tensor([f.label_ids for f in parsing_ptb_features], dtype=torch.long)

    parsing_ptb_dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids)

    return pos_dataset, ner_dataset, chunking_dataset, srl_dataset, onto_pos_dataset, onto_ner_dataset, parsing_ud_dataset, parsing_ptb_dataset
...
def load_and_cache_train_examples(args, tokenizer, pos_labels, ner_labels, chunking_labels, srl_labels, onto_pos_labels, onto_ner_labels, pad_token_label_id):
    ...
    parsing_ud_features_batches = []
    cnt = 0
    while cnt + mini_batch_size < len(parsing_ud_features):
        batch_t = []
        for i in range(cnt, cnt+mini_batch_size):
            batch_t.append(parsing_ud_features[i])
        parsing_ud_features_batches.append(batch_t)
        cnt += mini_batch_size
    batch_t = []
    for i in range(cnt, len(parsing_ud_features)):
        batch_t.append(parsing_ud_features[i])
    parsing_ud_features_batches.append(batch_t)

    parsing_ptb_features_batches = []
    cnt = 0
    while cnt + mini_batch_size < len(parsing_ptb_features):
        batch_t = []
        for i in range(cnt, cnt+mini_batch_size):
            batch_t.append(parsing_ptb_features[i])
        parsing_ptb_features_batches.append(batch_t)
        cnt += mini_batch_size
    batch_t = []
    for i in range(cnt, len(parsing_ptb_features)):
        batch_t.append(parsing_ptb_features[i])
    parsing_ptb_features_batches.append(batch_t)

    
    data_list = [pos_features_batchs, ner_features_batchs, chunking_features_batches, srl_features_batches, onto_pos_features_batches, onto_ner_features_batches, parsing_ud_features_batches, parsing_ptb_features_batches]
    return data_list
...
def evaluate(args, model, tokenizer, eval_dataset, labels, pad_token_label_id, mode, prefix=" ", task="pos"):
    ...
    if task == "pos":
        ...
        results = {
            ...
            "onto_ner_precision":precision_score(out_label_list, preds_list),
            "onto_ner_recall": recall_score(out_label_list, preds_list)
        }
    elif task == "parsing_ud":
        results = {
            "parsing_ud_UAS":accuracy_score(out_label_list, preds_list)
        }
    elif task == "parsing_ptb":
        results = {
            "parsing_ptb_UAS":accuracy_score(out_label_list, preds_list)
        }
...
def main():
    ...
    parser.add_argument("--parsing_ud_data_dir", type=str, default="")
    parser.add_argument("--parsing_ptb_data_dir", type=str, default="")
