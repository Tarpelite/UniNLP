...
class InputExample(object):
    ...
    def __init__(self, guid, words, labels_pos, labels_ner):
    ...
    with open(file_path, encoding="utf-8") as f:
        ...
        for line in f:
            if line == "\n":
                ...
                labels_pos.append(splits[1])
                if len(splits) > 1:
                    labels_ner.append(splits[-1])
                else:
...
def convert_examples_to_features_pos(examples,
                                 label_list,
                                 max_seq_length,
                                 tokenizer,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        tokens = []
        label_ids = []
        for word, label in zip(example.words, example.labels_pos):
...
def convert_examples_to_features_ner(examples,
                                 label_list,
                                 max_seq_length,
                                 tokenizer,
                                 cls_token_at_end=False,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        tokens = []
        label_ids = []
        for word, label in zip(example.words, example.labels_ner):
            word_tokens = tokenizer.tokenize(word)
            tokens.extend(word_tokens)
            # Use the real label id for the first token of the word, and padding ids for the remaining tokens
            label_ids.extend([label_map[label]] + [pad_token_label_id] * (len(word_tokens) - 1))
