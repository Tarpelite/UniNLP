...
from utils_parsing_v2 import convert_examples_to_features, get_labels, read_examples_from_file
import torch.nn as nn
from torch.optim import Adam

from transformers import AdamW, get_linear_schedule_with_warmup
from transformers import WEIGHTS_NAME, BertConfig, BertForTokenClassification, BertTokenizer, BertForParsingV2
...
MODEL_CLASSES = {
    "bert": (BertConfig, BertForParsingV2, BertTokenizer),
...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            if args.model_type != "distilbert":
                inputs["token_type_ids"]: batch[2] if args.model_type in ["bert", "xlnet"] else None  # XLM and RoBERTa don"t use segment_ids
            outputs = model(**inputs)
            tmp_eval_loss, logits_arc, logits_label = outputs[:3]
        ...
        nb_eval_steps += 1
        if preds_arc is None and preds_label is None:
            preds_arc = logits_arc.detach().cpu().numpy()
            preds_label = logits_label.detach().cpu().numpy()
            out_head_ids = inputs["heads"].detach().cpu().numpy()
    ...
    return results, preds_label_list
...
def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        features = convert_examples_to_features(examples, args.max_seq_length, tokenizer, label_list=labels,
    ...
    all_heads_ids = torch.tensor([f.head_ids for f in features], dtype=torch.long)
    all_label_ids = torch.tensor([f.label_ids for f in features], dtype=torch.long)

    dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_heads_ids, all_label_ids)
    return dataset
