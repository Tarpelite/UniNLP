...
def load_and_cache_dev_examples(args, tokenizer, pos_labels, ner_labels, pad_token_label_id, is_ft=False):
    ...
    return pos_dataset, ner_dataset

def load_and_cache_train_examples(args, tokenizer, pos_labels, ner_labels, pad_token_label_id):
...
def train(args, train_data_list, model, tokenizer, labels_pos, labels_ner, pad_token_label_id):
    ...
    for _ in train_iterator:
        ...
        for step, task_id in enumerate(epoch_iterator):
            ...
            inputs = {"input_ids":input_ids, "attention_mask":input_mask, "labels":label_ids, "task_id":task_id}
            outputs = model(**inputs)
            loss = outputs[0]     
...
def evaluate(args, model, tokenizer, eval_dataset, labels, pad_token_label_id, mode, prefix=" ", task="pos"):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            inputs = {"input_ids": batch[0],
                      "attention_mask": batch[1],
                      "labels": batch[3], "task_id":task_id}
            if args.model_type != "distilbert":
                inputs["token_type_ids"]: batch[2] if args.model_type in ["bert", "xlnet"] else None  # XLM and RoBERTa don"t use segment_ids
            outputs = model(**inputs)
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            result, _ = evaluate(args, model_pos, tokenizer, pos_dataset, labels_pos, pad_token_label_id, mode="dev", prefix=global_step, task="pos")

            _, _, model_ner = finetune(args, ner_dataset_ft, model, tokenizer,  labels_ner, pad_token_label_id)
            result, _ = evaluate(args, model_ner, tokenizer, ner_dataset, labels_ner, pad_token_label_id, mode="dev", prefix=global_step, task="ner")
