...
class RecurrentEncoder(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        h0 = torch.randn(self.num_layers*self.num_directions, batch, self.hidden_size).cuda()
        c0 = torch.randn(self.num_layers*self.num_directions, batch, self.hidden_size).cuda()
...
class BiAffineParser(BertPreTrainedModel):
    ...
    def __init__(self, config, mlp_input, mlp_arc_hidden,
        ...
        self.encoder = RecurrentEncoder(config.hidden_size, config.hidden_size*2, 1, batch_first=True, dropout=config.hidden_dropout_prob)
        # Arc MLPs
